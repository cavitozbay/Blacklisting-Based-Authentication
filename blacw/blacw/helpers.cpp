...
NTL::ZZ_pX poly_to_zzpX(Poly& f)
{
    char ord_str[] = "16798108731015832284940804142231733909759579603404752749028378864165570215949";
    
    NTL::ZZ p = NTL::to_ZZ(ord_str);
    ...
    for (size_t i = 0; i <= degree(f); i++)
    {
        ...
        ss >> tZZ;
        tZZ_p = to_ZZ_p(tZZ);
        SetCoeff(aNTL, long(i), tZZ_p);
    }
    mip->IOBASE = 16;
    return aNTL;
}

void egcd_fast_vec_zzp(vector<Big>& d, vector<Big>& s, vector<Big>& t, NTL::ZZ_pX& a, NTL::ZZ_pX& b)
{
    ...
    NTL::ZZ_pX dNTL, sNTL, tNTL;

    NTL::XGCD(dNTL, sNTL, tNTL, a, b);

    // cout << deg(dNTL) << endl;
    // cout << dNTL << endl;
    // cout << deg(sNTL) << endl;
    term* pos = __null;
    for (size_t i = 0; i <= NTL::deg(dNTL); i++)
    {
        tZZ_p = NTL::coeff(dNTL, long(i));
        // cout << tZZ_p << endl;
        stringstream ss;
        ss << tZZ_p;
        // cout << a << endl;
    ...
    pos = __null;
    for (size_t i = 0; i <= deg(sNTL); i++)
    {
        tZZ_p = coeff(sNTL, long(i));
        stringstream ss;
        ss << tZZ_p;
        ss >> tBig;
        s.push_back(tBig);
    ...
    pos = __null;
    for (size_t i = 0; i <= deg(tNTL); i++)
    {
        tZZ_p = coeff(tNTL, long(i));
        stringstream ss;
        ss << tZZ_p;
        ss >> tBig;
        t.push_back(tBig);
