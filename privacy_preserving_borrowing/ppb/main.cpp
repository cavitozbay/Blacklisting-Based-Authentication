...
void test_pcomp_protocol()
{
    ...
    PPBIssuer issuer(bn, g, g_hat, tlp);
    //PPBHolder holder(bn, g, g_hat, issuer.com, issuer.acc, issuer.spseq, issuer.w, issuer.g_tilde, issuer.h_tilde);
    PPBHolder holder(bn, issuer);
...
void test_tlp()
{
    ...
    f.close();
    ifstream file("ppb_tlp.bin", ios::binary);
...
int main()
{
    ...
    ACC_SIZE = 4031;
    //bmark_pcomp();
    for (INITIAL_DEBT_COUNT = 0; INITIAL_DEBT_COUNT < 30; INITIAL_DEBT_COUNT++)
    {
        bmark_all();
    }*/
    //bmark_pcomp();
